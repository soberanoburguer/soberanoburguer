...
// Função para calcular se a loja está aberta baseado no horário padrão (17:30 - 00:00)
  ...
  return (
    <div className="relative min-h-screen bg-brand-charcoal text-white flex flex-col justify-between overflow-hidden">
      ...
      <header className="w-full max-w-7xl mx-auto px-6 py-6 flex items-center justify-between z-10">
        ...
        <span className="font-label text-[10px] font-bold tracking-[0.25em] uppercase text-brand-amber bg-black/40 border border-brand-amber/20 px-3.5 py-1.5 rounded backdrop-blur-md">
          O Reinado do Sabor
        </span>
      </header>

      {/* Toast de Visita Anterior */}
      {lastVisited && (
        <div className="w-full max-w-xl mx-auto px-4 z-20 animate-in fade-in slide-in-from-top duration-500">
          <div className="bg-black/60 border border-brand-amber/20 backdrop-blur-xl rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-2xl">
            <div className="flex items-center gap-3">
              <span className="w-9 h-9 rounded-full bg-brand-amber/10 flex items-center justify-center text-brand-amber shrink-0">
                <Sparkles className="w-4 h-4" />
              </span>
              <p className="text-sm text-white/80 text-center sm:text-left">
                Você visitou <strong className="text-white">{lastVisited.cidade}</strong> recentemente.
              </p>
            </div>
            <Link
              href={`/${lastVisited.slug}`}
              onClick={() => handleSelect(lastVisited.slug)}
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-soberano-gradient text-white text-xs font-black uppercase tracking-wider hover:opacity-90 transition-opacity whitespace-nowrap"
            >
              Ir para lá
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-12 z-10 max-w-5xl mx-auto w-full">
        ...
        <div className="text-center mb-10 max-w-2xl">
          <h1 className="font-heading text-5xl md:text-7xl leading-none uppercase tracking-[0.01em] mb-4">
            Escolha seu <span className="text-soberano-gradient">Reinado</span>
          </h1>
          <p className="text-white/60 text-sm md:text-base leading-relaxed">
            Selecione a unidade mais próxima de você para acessar o cardápio local, horários de funcionamento e fazer seu pedido online.
          </p>
        </div>
      ...
      {/* Footer */}
      <footer className="w-full max-w-7xl mx-auto px-6 py-8 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-4 z-10 text-center md:text-left">
